...
const ParentLogin = () => {
  ...
  const onLogin = async (values) => {
    setLoading(true)
    history('/parents/dashboard')
    // TODO: commenting for now
    //api.post('/users/parents/login', { ...values, token })
    //    .then(() => {
    //        setData({
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: "80vh" }}>
        <div className="parent-login" style={{ width: '500px', margin: '0 auto', background: "white", color: "black" }}>
          <Title level={2}>Parent Login</Title>
          <Form name="login" onFinish={onLogin}>
            ...
            <Form.Item className='button-float'>
              ...
              {/*<div style={{ marginTop: '16px', textAlign: 'center' }}>
                                New User?&nbsp;&nbsp;  <Link to="/parents/register">Sign up</Link>
                            </div>*/}
            </Form.Item>
          </Form>
        </div>
